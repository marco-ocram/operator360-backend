...
func GetMediumRiskOperators(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[GetMediumRiskOperators] S3 client error user=%s: %v", user.ADID, err)
	...
	if err != nil {
		log.Printf("[GetMediumRiskOperators] S3 fetch failed key=%s user=%s: %v", fileName, user.ADID, err)
	...
	if err != nil {
		log.Printf("[GetMediumRiskOperators] Read body failed key=%s: %v", fileName, err)
	...
	if err != nil {
		log.Printf("[GetMediumRiskOperators] CreateTemp error: %v", err)
	...
	if _, err = tempFile.Write(parquetBytes); err != nil {
		...
		log.Printf("[GetMediumRiskOperators] Write temp error: %v", err)
	...
	if err != nil {
		log.Printf("[GetMediumRiskOperators] Open parquet error: %v", err)
	...
	if err != nil {
		log.Printf("[GetMediumRiskOperators] Parquet reader error: %v", err)
	...
	if err != nil {
		log.Printf("[GetMediumRiskOperators] Read parquet data error: %v", err)
